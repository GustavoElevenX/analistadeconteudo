...
export function getLatestPatterns() {
  ...
  return {
    ...row,
    weight_vamo: Number(row.weight_vendas || 0),
    weight_familia: Number(row.weight_vida || 0),
    weight_oferta: Math.max(0, 1 - (
      Number(row.weight_vendas || 0)
      + Number(row.weight_empreendedorismo || 0)
      + Number(row.weight_fe || 0)
      + Number(row.weight_vida || 0)
    )),
