...
function calculateWeights(stats) {
  ...
  const funil = normalize(funilScores, { topo: 0.35, meio: 0.40, fundo: 0.25 });
  const pillar = normalize(pillarScores, { vendas: 0.55, empreendedorismo: 0.25, fe: 0.15, vida: 0.05 });
...
function compositeScore(row) {
  return (Number(row.avg_lead_score || 0) * 0.5)
    + (Number(row.avg_reach || 0) * 0.3)
    + (Number(row.avg_engagement_rate || 0) * 0.2);
}
